...
class fm_learn_mcmc : public fm_learn {
	...
	protected:
		...
		sparse_row<float> empty_data_row; // this is a dummy row for attributes that do not exist in the training data (but in test data)		
	...
	protected:
		...
		void draw_w(double& w, double& w_mu, double& w_lambda, sparse_row<float>& feature_data) {
		...
		void draw_w_rel(double& w, double& w_mu, double& w_lambda, sparse_row<float>& feature_data, relation_cache* r_cache) {
			double w_sigma_sqr = 0;
			double w_mean = 0;
			// w_sigma_sqr = \sum h^2
			// w_mean = \sum h*e
			...
			for (uint i_fd = 0; i_fd < feature_data.size; i_fd++) {	
				...
				num_all += static_cast<uint>(r_cache[train_case_index].wnum);
			}
		...
		void draw_v(double& v, double& v_mu, double& v_lambda, sparse_row<float>& feature_data) {
		...
		void draw_v_rel(double& v, double& v_mu, double& v_lambda, sparse_row<float>& feature_data, relation_cache* r_cache) {
			double v_sigma_sqr = 0;
			double v_mean = 0;
			// v_sigma_sqr = \sum h^2
			// v_mean = \sum h*e
	...
	public:
		...
		virtual void learn(Data& train, Data& test) {
			...
			//MemoryLog::getInstance().log_new("e_q_term", sizeof(e_q_term), train.num_cases);
			cache = new e_q_term[train.num_cases];
			//MemoryLog::getInstance().log_new("e_q_term", sizeof(e_q_term), test.num_cases);
			cache_test = new e_q_term[test.num_cases];

			rel_cache.setSize(train.relation.dim);
			for (uint r = 0; r < train.relation.dim; r++) {
				//MemoryLog::getInstance().log_new("relation_cache", sizeof(relation_cache), train.relation(r).data->num_cases);
			...
			for (uint i = 0; i < train.relation.dim; i++) {
				//MemoryLog::getInstance().log_free("relation_cache", sizeof(relation_cache), train.relation(i).data->num_cases);
				delete[] rel_cache(i);
			}
			//MemoryLog::getInstance().log_free("e_q_term", sizeof(e_q_term), test.num_cases);
			delete[] cache_test;
			//MemoryLog::getInstance().log_free("e_q_term", sizeof(e_q_term), train.num_cases);
			delete[] cache;
		}
