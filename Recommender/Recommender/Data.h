...
void Data::load(std::string filename) {
	...
	/*int load_from = 0;
	if ((! has_x || file_exists(filename + ".data")) && (! has_xt || file_exists(filename + ".datat")) && file_exists(filename + ".target")) {
		load_from = 1;
	} else if ((! has_x || file_exists(filename + ".x")) && (! has_xt || file_exists(filename + ".xt")) && file_exists(filename + ".y")) {
		load_from = 2;
	}
	...
	if (load_from > 0) {
		...
		if (has_x) {
			std::cout << "data... ";
			if (load_from == 1) {
				this->data = new LargeSparseMatrixHD<float>(filename + ".data", this_cs);
			} else {
		...
		if (has_xt) {
			...
			if (load_from == 1) {
				this->data_t = new LargeSparseMatrixHD<float>(filename + ".datat", this_cs);
			} else {
		...
		min_target = +std::numeric_limits<float>::max();
		max_target = -std::numeric_limits<float>::max();
	...
	this->data = new LargeSparseMatrixMemory<float>();
	
	DVector< sparse_row<float> >& data = ((LargeSparseMatrixMemory<float>*)this->data)->data;
	...
	// (1) determine the number of rows and the maximum feature_id
	{
		...
		while (!fData.eof()) {
			...
			if (sscanf(pline, "%f%n", &_value, &nchar) >=1) {
				...
				while ((*pline != 0) && ((*pline == ' ')  || (*pline == 9))) {  // skip trailing spaces
					pline++; 
				}
				if ((*pline != 0)  && (*pline != '#')) { 
	...
	((LargeSparseMatrixMemory<float>*)this->data)->num_cols = num_feature;
	((LargeSparseMatrixMemory<float>*)this->data)->num_values = num_values;

	//MemoryLog::getInstance().log_new("data_float", sizeof(sparse_entry<float>), num_values);			
	sparse_entry<float>* cache = new sparse_entry<float>[static_cast<uint>(num_values)];
...
void Data::create_data_t() {
	...
	data_t = new LargeSparseMatrixMemory<float>();

	DVector< sparse_row<float> >& data_t = ((LargeSparseMatrixMemory<float>*)this->data_t)->data;
	...
	((LargeSparseMatrixMemory<float>*)this->data_t)->num_cols = data.dim;
	((LargeSparseMatrixMemory<float>*)this->data_t)->num_values = num_values;
