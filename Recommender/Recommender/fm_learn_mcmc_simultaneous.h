	...
	protected:
		...
		virtual void _learn(Data& train, Data& test) {
			...
			for (uint i = num_complete_iter; i < num_iter; i++) {
				double iteration_time = get_user_time();
				clock_t iteration_time3 = clock();
				double iteration_time4 = static_cast<double>(time(nullptr));
				...
				iteration_time = (get_user_time() - iteration_time);
				iteration_time3 = clock() - iteration_time3;
				iteration_time4 = (time(nullptr) - iteration_time4);
				if (log != nullptr) {
		...
		void _evaluate(DVector<double>& pred, DVector<float>& target, double normalizer, double& rmse, double& mae, uint from_case, uint to_case) {
		...
		void _evaluate_class(DVector<double>& pred, DVector<float>& target, double normalizer, double& accuracy, double& loglikelihood, uint from_case, uint to_case) {
		...
		void _evaluate(DVector<double>& pred, DVector<float>& target, double normalizer, double& rmse, double& mae, uint& num_eval_cases) {
			_evaluate(pred, target, normalizer, rmse, mae, 0, num_eval_cases);
		}

		void _evaluate_class(DVector<double>& pred, DVector<float>& target, double normalizer, double& accuracy, double& loglikelihood, uint& num_eval_cases) {
